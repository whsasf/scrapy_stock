...
class Prepare(object):
    ...
    def fetch_rate():
        ...
        try:
            ...
            #print('data_stream2',data_stream2)
...
class MongoDBPipeline(object):
    ...
    def close_spider(self,spider):
        ...
        raw_data_list = list(self.db.stock_latest.aggregate([{"$lookup":{"from":"stock_info","localField":"stock_id","foreignField":"stock_id","as":"result"}},{"$addFields":{"stock_buss_alias":"$result.stock_buss_alias"}},{"$project":{"_id":0,"stock_id":1,"stock_area":1,"stock_come":1,"stock_name":1,"stock_value":1,"stock_buss_alias":1}}]))
        ...
        # add stock_percent = stock_value - stock_value_pre(yesterday or latest day)
        #(1) get stock_value_pre
        def getStockValuePre(sid):
           flag1 = datetime.datetime.now().strftime('%p')
           ii = 1
           stock_value_pre = 0
           while ii < 30:
               temp_day = str(datetime.date.today() -datetime.timedelta(days=ii))+'-'+flag1
               temp_result = list(self.db.stock.find({'stock_id':sid,'time_stamp':temp_day},{"_id":0,'stock_value':1}))
        ...
        # add NO and stock_stock_pre for CN
        for indexxx  in range(0,len(sort_data_list_CN)):
            ...
            # add stock_stock_pre
            pre_value = getStockValuePre(sort_data_list_CN[indexxx]['stock_id'])
            temp_percent = (sort_data_list_CN[indexxx]['stock_value'] - pre_value )/ pre_value
            sort_data_list_CN[indexxx]['stock_percent'] = int(temp_percent * 10000)/10000
        # add NO and stock_stock_pre for US
        for indexxx  in range(0,len(sort_data_list_US)):
            ...
            # add stock_stock_pre
            pre_value = getStockValuePre(sort_data_list_US[indexxx]['stock_id'])
            temp_percent = (sort_data_list_US[indexxx]['stock_value'] - pre_value )/ pre_value
            sort_data_list_US[indexxx]['stock_percent'] = int(temp_percent * 10000)/10000
        ...
        def echart():
            ...
            for date in date_range: 
                temp1 = date+'-PM'
                temp2 = date+'-AM'
                if self.db.stock.find({"time_stamp":temp1}).count() > 0:
                    raw_data = list(self.db.stock.find({"time_stamp":temp1,"stock_value":{"$gt":0}},{"stock_name":1,"stock_area":1,"stock_value":1,"_id":0}))
                else:
                    raw_data = list(self.db.stock.find({"time_stamp":temp2,"stock_value":{"$gt":0}},{"stock_name":1,"stock_area":1,"stock_value":1,"_id":0}))
                if raw_data:
        ...
        # CN
        with open('files/rankdatacn.json','w',encoding='utf-8')  as filehander:
            json.dump(sort_data_list_CN,filehander,ensure_ascii=False,indent=4)
        #US
        with open('files/rankdataus.json','w',encoding='utf-8')  as filehander:
            json.dump(sort_data_list_US,filehander,ensure_ascii=False,indent=4)
        # allrank
        with open('files/allrank.json','w',encoding='utf-8')  as filehander:
    ...
    def insert_db(self,item):
        ...
        self.db.stock.update_one({"stock_id": item['stock_id'],"time_stamp":item['time_stamp']},{"$set":{"stock_name": item['stock_name'],"stock_value": item['stock_value'],"stock_id": item['stock_id'],"time_stamp":item['time_stamp'],"stock_area":item['stock_area'],"stock_come":item['stock_come']}},True)

        # inset into stock_latest collection ,this collection contains the latest stock data
        item['status_flag'] = 0
        self.db.stock_latest.update_one({"stock_id": item['stock_id'],"status_flag":item['status_flag']},{"$set":{"stock_name": item['stock_name'],"stock_value": item['stock_value'],"status_flag":item['status_flag'],"stock_id": item['stock_id'],"time_stamp":item['time_stamp'],"stock_area":item['stock_area'],"stock_come":item['stock_come']}},True)
